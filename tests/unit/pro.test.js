...
const boot = (opts) => {
  const o = opts || {};
  global.window = {
    // This suite covers the SHARED store session, so it sets the Bank
    // Connect gate explicitly. That Pro registers alone when the gate is off
    // is asserted in tests/unit/bankConnectGate.test.js.
