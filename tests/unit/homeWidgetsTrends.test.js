...
describe('savings widget', () => {
  ...
  it('agrees with incomeExpense everywhere — both whole-month/EOM (v0.94)', () => {
    seed([
      { type: 'income', categoryId: 'cat_salary', amount: 2000, date: monthDay(0, 1) },
      { type: 'expense', amount: 800, date: monthDay(0, 2) }
    ]);
    // With only past-dated rows the two report the same net (as always).
    expect(renderOne('savings', 'small', {}).html).toContain('$1,200.00');
    expect(renderOne('incomeExpense', 'small', {}).html).toContain('$1,200.00');

    // v0.94: savings joined incomeExpense on the EOM side — a scheduled row
    // later this month lands in BOTH widgets, so they can never disagree
    // about the same month (the v0.82 MTD divergence is gone). This also
    // makes the assertion date-independent: it no longer matters whether the
    // suite runs before or after the 25th.
    seed([{ type: 'expense', amount: 300, date: monthDay(0, 25) }]);
    expect(renderOne('savings', 'small', {}).html).toContain('$900.00');
