...
> Status: **B1–B7 SHIPPED 2026-09-07 (broker live on staging; app v1.05–v1.11; §16 = B7 as built). B8 (native wiring, v1.12) is BUILT in §16.7 and waits for its device walk — the remaining owner steps are listed there.** See §9–§16.
...
### 16.7 B8 as built — native wiring, v1.12, 2026-09-07 (device walk pending)

Everything that can be done without a device is in the repo; the rest is a
short owner checklist (below). Not Bank Connect logic — the wiring the B3
return leg, B5 store flow and D-C15 token storage assumed.

**Plugins (`package.json` → `npx cap sync android`).**
`@capacitor/browser` 6.0.6 (the bank's SCA page, Custom Tabs /
SFSafariViewController — `openSca` / `closeSca` already used it),
`@aparajita/capacitor-secure-storage` 6.0.1 (D-C15) and the existing
`cordova-plugin-purchase` 13.18 are now registered: `capacitor.settings.gradle`
+ `capacitor.build.gradle` carry the three projects, the purchase plugin
brings `com.android.billingclient:billing:9.0.0` and the `BILLING` permission
through the gitignored `android/capacitor-cordova-android-plugins/`
(regenerated by every `npx cap sync android`, which also needs a `dist/`
from `npm run build`). iOS: `Podfile` lists `CapacitorBrowser`,
`AparajitaCapacitorSecureStorage` and `CordovaPlugins`; `npx cap sync ios`
(pod install) is a Mac step.

**Token storage fix (`src/bank-connect.js`).** The B3 adapter tested for
`ss.get`, but the SecureStorage plugin's bridge proxy only exposes its
NATIVE methods (`internalGetItem` / `internalSetItem` / `internalRemoveItem`,
keyed by `prefixedKey`, JSON-encoded `data`) — the `get`/`set` API lives in a
JS wrapper this no-bundler app never loads. On a real device the token would
have fallen back to localStorage silently. `_secureKind` now detects the
plugin shape (`aparajita` or the older `capacitor-secure-storage-plugin`
`get/set/remove`), `_secureRead` / `_secureWrite` speak each one, the key is
`capacitor-storage_stackd_device_token` (the wrapper's prefix, so both read
the same item), and `tokenGet` migrates a token a pre-B8 build left in
localStorage into the keystore once, then deletes the plain copy.

**Android (`android/app/src/main/AndroidManifest.xml`).** Two
`android:autoVerify="true"` VIEW filters on `MainActivity` (already
`singleTask`, so a warm return arrives as `appUrlOpen`): `https://
api.stackdplatform.com/v1/connect/return` and the same on `api-staging` —
one filter per host, because before Android 12 a filter is verified as a
whole and staging (no fingerprint yet) must not break production. A third
filter takes the `stackd://connect/...` fallback from the broker's hand-off
page. `main.js` (`appUrlOpen` + `getLaunchUrl`) and
`BankConnect.parseReturnUrl` were already built for both shapes.

**iOS (`ios/App/`).** `Info.plist` gains the `stackd` URL scheme;
`App/App.entitlements` (new, referenced by `CODE_SIGN_ENTITLEMENTS` in both
build configurations) carries `applinks:api.stackdplatform.com` and
`applinks:api-staging.stackdplatform.com`. Capacitor's `AppDelegate`
already forwards `continueUserActivity` / `openURL` to the App plugin.

**Tests.** `tests/unit/nativeWiring.test.js` (B8): the token adapter against
a fake of the plugin's real native surface (set → JSON under the prefixed
key, get, clear, never localStorage), the legacy plugin shape, the one-time
migration, and the manifest / plist / entitlements / Podfile / package.json
contracts (both hosts with `autoVerify` and the exact path, the `stackd`
scheme on both platforms, the three plugins) cross-checked against
`parseReturnUrl` so a wiring edit cannot drift from the parser.

**Owner checklist (needs the machine with the Android SDK / a Mac).**

1. `npm install && npm run build && npx cap sync android`, then open
   `android/` in Android Studio and build a debug APK. First gradle sync
   pulls the billing library. (The gradle/emulator quirks memory applies.)
2. Fingerprints: the debug keystore's SHA-256 into staging's
   `ANDROID_SHA256_FINGERPRINTS`, the release / Play App Signing keys into
   production's (README "Deploy production" has the `keytool` lines);
   `npm run deploy:staging`; confirm `/.well-known/assetlinks.json` shows it.
3. Device walk of §3.6 (staging, sandbox "Mock ASPSP"): install, `adb shell
   pm get-app-links com.stackd.finance` → `verified`; connect a bank →
   Custom Tab → the bank returns → the app opens from the link (warm), then
   kill the app during the bank step and repeat (cold start from the link →
   `getLaunchUrl` → `pendingRef` resume). Then with verification disabled
   (`adb shell pm set-app-links --package com.stackd.finance 0 all`) to see
   the hand-off page → `stackd://` path.
4. Token: after the first connect, `adb shell run-as com.stackd.finance ls
   shared_prefs` shows `WSSecureStorageSharedPreferences.xml` and
   `stackd_device_token` is absent from the WebView's localStorage.
5. Store: `cordova-plugin-purchase` needs the app on an internal-testing
   track with the two products created (§14) before `initStore()` returns
   prices; until then the paywall says prices are loading.
6. Mac: `npx cap sync ios`, enable *Associated Domains* on the App ID, set
   `IOS_APP_ID` (`TEAMID.com.stackd.finance`) on the broker, repeat step 3.
7. `android/app/build.gradle` still says `versionName "1.0"` / `versionCode
   1`; bump both before the first store upload.
